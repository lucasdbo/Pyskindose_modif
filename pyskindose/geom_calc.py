...
def fetch_and_append_hvl(data_norm: pd.DataFrame, inherent_filtration: float) -> pd.DataFrame:
    ...
    for event in range(len(data_norm)):
        kV = data_norm.loc[event, "kVp"]
        Al = data_norm.loc[event, "filter_thickness_Al"]
        Cu = data_norm.loc[event, "filter_thickness_Cu"]

        _hvl = hvl_data.loc[(hvl_data["kvp_kv"] == kV) 
