...
def RDM_normalizer(data_parsed: pd.DataFrame, settings: PyskindoseSettings)  -> pd.DataFrame:

    data_parsed = data_parsed.loc[data_parsed["Type d'IR"] != "Rotational Acquisition"]
    data_parsed = data_parsed.loc[data_parsed["Distance source-detecteur (mm)"] != str(0)]
    data_parsed = data_parsed.loc[data_parsed["Distance source-detecteur (mm)"] != str(0)]
    data_parsed = data_parsed.dropna(subset="KVP (kV)")

    n = len(data_parsed["KVP (kV)"])

    data_norm = pd.DataFrame(index=[i for i in range(n)])

    data_norm[KEY_NORMALIZATION_MODEL_NAME] = "AlluraClarity"
    data_norm[KEY_NORMALIZATION_DISTANCE_SOURCE_DETECTOR] = data_parsed["Distance source-detecteur (mm)"] / 10
    data_norm[KEY_NORMALIZATION_DISTANCE_SOURCE_ISOCENTER] = data_parsed["Distance source-isocentre (mm)"] / 10
    data_norm[KEY_NORMALIZATION_DISTANCE_ISOCENTER_DETECTOR] = data_norm[KEY_NORMALIZATION_DISTANCE_SOURCE_DETECTOR] - data_norm[KEY_NORMALIZATION_DISTANCE_SOURCE_ISOCENTER]
    data_norm[KEY_NORMALIZATION_DISTANCE_SOURCE_IRP] = data_norm[KEY_NORMALIZATION_DISTANCE_SOURCE_ISOCENTER] - 15
    data_norm[KEY_NORMALIZATION_ACQUISITION_TYPE] = data_parsed["Type d'IR"]
    data_norm[KEY_NORMALIZATION_ACQUISITION_PLANE] = data_parsed["Plan d'acquisition"]

    data_norm["Tx"] = data_parsed["Position longitudinale de la table (mm)"] / 10
    data_norm["Tz"] = data_parsed["Position latérale de la table (mm)"] / 10 
    data_norm["Ty"] = 106 - (data_parsed["Hauteur de la table (mm)"] / 10)

    data_norm["At1"] = [0 for _ in range(n)]
    data_norm["At2"] = [0 for _ in range(n)]
    data_norm["At3"] = [0 for _ in range(n)]

    data_norm["Ap1"] = data_parsed["Angle primaire du pos. (°)"]
    data_norm["Ap2"] = data_parsed["Angle secondaire du pos. (°)"]
    data_norm["Ap3"] = [0 for _ in range(n)]
    
    data_norm["DSL"] = [35 for _ in range(n)]

    data_norm["FS_lat"] = data_parsed["Côté carré équivalent (cm)"]
    data_norm["FS_long"] = data_parsed["Côté carré équivalent (cm)"]

    data_norm[KEY_NORMALIZATION_KVP] = data_parsed["KVP (kV)"]
    filter = data_parsed["Epaisseur min du filtre X-Ray"]
    data_norm[KEY_NORMALIZATION_FILTER_SIZE_ALUMINUM] = [float(ele.split("\\")[0]) 
                                                         if isinstance(ele, str) else 0 for ele in filter]
    data_norm[KEY_NORMALIZATION_FILTER_SIZE_COPPER] = [float(ele.split("\\")[1].replace(",", ".")) 
                                                       if isinstance(ele, str) else 0 for ele in filter]
    data_norm[KEY_NORMALIZATION_AIR_KERMA] = data_parsed["Air Kerma (mGy)"]
