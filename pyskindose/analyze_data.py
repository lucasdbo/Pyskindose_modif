...
def analyze_data(
    ...
    Dose_map = output[c.OUTPUT_KEY_DOSE_MAP].copy()
    Dose_map[Dose_map < 0.5*np.nanmax(Dose_map)] = np.nan

    print(np.nanmax(Dose_map), np.nanmin(Dose_map))
    print(np.nanmean(Dose_map))
    print(np.count_nonzero(~np.isnan(Dose_map))/len(output[c.OUTPUT_KEY_DOSE_MAP])*100)

    import matplotlib.pyplot as plt
    
    # fig, ax = plt.subplots()
    # cm = plt.get_cmap('turbo')
    # edges, bin, patches = ax.hist(output[c.OUTPUT_KEY_DOSE_MAP][output[c.OUTPUT_KEY_DOSE_MAP]>0], 50, histtype="stepfilled", cumulative=-1)
    # for i, p in enumerate(patches):
    #     plt.setp(p, 'facecolor', cm(i/50))
    # plt.show()
    plt.hist(output[c.OUTPUT_KEY_DOSE_MAP][output[c.OUTPUT_KEY_DOSE_MAP]>0], 50, histtype="stepfilled", cumulative=-1)
